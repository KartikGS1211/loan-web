...
export const step1Schema = z.object({
  loanAmount: z.number().min(50000, 'Minimum loan amount is 50,000').max(5000000, 'Maximum loan amount is 50,00,000'),
  loanTenure: z.number().min(12, 'Minimum tenure is 12 months').max(60, 'Maximum tenure is 60 months'),
...
export const step5Schema = z.object({
  ...
  if (data.employmentType === 'salaried') {
    if (!data.companyName || data.companyName.trim().length < 2) {
      ctx.addIssue({ path: ['companyName'], message: 'Company name is required', code: z.ZodIssueCode.custom });
    }
    if (!data.monthlyIncome || data.monthlyIncome < 15000) {
      ctx.addIssue({ path: ['monthlyIncome'], message: 'Minimum monthly income must be 15,000', code: z.ZodIssueCode.custom });
    }
  } else {
    if (!data.businessName || data.businessName.trim().length < 2) {
      ctx.addIssue({ path: ['businessName'], message: 'Business name is required', code: z.ZodIssueCode.custom });
    }
    if (!data.annualTurnover || data.annualTurnover < 200000) {
      ctx.addIssue({ path: ['annualTurnover'], message: 'Minimum annual turnover must be 2,00,000', code: z.ZodIssueCode.custom });
    }
  }
});
...
export const step6Schema = z.object({
  ...
  coApplicantPan: z.string().regex(/^[A-Za-z]{5}[0-9]{4}[A-Za-z]{1}$/, 'Invalid PAN format'),
